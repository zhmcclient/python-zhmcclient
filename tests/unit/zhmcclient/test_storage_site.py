...
class TestStorageSite:
    ...
    def test_storage_site_list_cpc_uri_qp(self):
        """
        Test that filtering by 'cpc-uris' sends 'cpc-uri' (singular) as the
        HMC query parameter, not 'cpc-uris'.

        The HMC API accepts 'cpc-uri' as the query parameter name even though
        the resource property is named 'cpc-uris'. The StorageSiteManager
        must map the property name to the correct query parameter name via
        _query_props_names.
        """
        other_cpc_uri = '/api/cpcs/other-cpc'
        faked_ss1 = self.add_storage_site(name=SS1_NAME, cpc_uris=[CPC_URI])
        self.add_storage_site(name=SS2_NAME, cpc_uris=[other_cpc_uri])

        ss_mgr = self.console.storage_sites

        # Verify the manager has the expected rename mapping
        # pylint: disable=protected-access
        assert ss_mgr._query_props_names == {'cpc-uris': 'cpc-uri'}

        # Filter by cpc-uris: only SS1 should be returned
        cpc_sites = ss_mgr.list(filter_args={'cpc-uris': CPC_URI})
        assert len(cpc_sites) == 1
        assert cpc_sites[0].name == faked_ss1.name

        # Filter by cpc-uris for the other CPC: only SS2 should be returned
        other_cpc_sites = ss_mgr.list(filter_args={'cpc-uris': other_cpc_uri})
        assert len(other_cpc_sites) == 1
        assert other_cpc_sites[0].name == SS2_NAME

    def test_storage_site_list_filter_cpc_uris(self):
        """
        Test that filtering by 'cpc-uris' returns only matching sites.

        The HMC query parameter for the 'cpc-uris' property is named
        'cpc-uri' (singular). The StorageSiteManager maps the property name
        to the correct query parameter name so that list() filtering works.
        """
        other_cpc_uri = '/api/cpcs/other-cpc'
        faked_ss1 = self.add_storage_site(name=SS1_NAME, cpc_uris=[CPC_URI])
        self.add_storage_site(name=SS2_NAME, cpc_uris=[other_cpc_uri])
