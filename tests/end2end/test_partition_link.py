...
from .utils import skip_warn, pick_test_resources, TEST_PREFIX, \
    ...
    runtest_get_properties, assert_properties
...
def test_partlink_crud(dpm_mode_cpcs, pl_type):  # noqa: F811
    ...
    for cpc in dpm_mode_cpcs:
        ...
        if pl_type == 'ctc':
            ...
            if not adapters:
                pytest.skip(f"CPC {cpc.name} has no FC adapters for "
    ...
    for cpc in dpm_mode_cpcs:
        ...
        if pl_type == 'ctc':
            ...
            if len(partitions) < 2:
                pytest.skip(f"CPC {cpc.name} has no two stopped partitions "
                            "for CTC partition link creation")
            partition_1, partition_2 = random.choices(partitions, k=2)
            replacements['partition_1'] = partition_1
            replacements['partition_2'] = partition_2
        ...
        try:
            ...
            if partlink:
                try:
                    ...
                    time.sleep(10)
                    print("Retrying PartitionLink.delete() ...")
                    partlink.delete()
