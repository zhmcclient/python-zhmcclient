...
* ``complete`` - All requests for attaching or detaching the partition link to
  or from partitions are complete.
...
class PartitionLinkManager(BaseManager):
    ...
    * for technology-specific support, see the API features described in
    ...
    def create(self, properties=None):
        ...
        :meth:`~zhmcclient.PartitionLink.attach_to_partition` and
        :meth:`~zhmcclient.PartitionLink.detach_from_partition`, and also with
        the more generic :meth:`~zhmcclient.PartitionLink.update_properties`.
        ...
        * Object-access permission to all FICON adapter objects used for the
        ...
        Returns:
          ...
          * If `wait_for_completion` is `True`, returns a
            :class:`~zhmcclient.PartitionLink` object representing the new
            partition link.
...
class PartitionLink(BaseResource):
    ...
    * for technology-specific support, see the API features described in
    ...
    def attach_to_partition(
        ...
        * Object-access permission to this partition link.
        ...
        Parameters:
          ...
          partition (:class:`~zhmcclient.Partition`): The partition to which
            this partition link is to be attached.

          properties (dict): Optional properties for controlling device
            numbers and other attributes.

            For Hipersockets and SMC-D-type partition links, the following
            properties can be used in this method. For a full description,
            see the description of the "Modify Partition Link" operation
            in the :term:`HMC API` book. The provided properties will become
            an item in the ``added-connections`` array parameter, along with
            the provided partition. The following properties can be specified:

              * ``number-of-added-nics`` - int - optional - Number of NICs to
                define in the partition.
              * ``added-nics`` - array of new-nic objects - optional:

                * ``device-numbers`` - array of string - optional - device
                  numbers
                * ``fid`` - int - optional - SMCD only: FID
                * ``vlan-id`` - int - optional - Hipersockets only: VLAN ID
                * ``mac-address`` - string - optional - Hipersockets only: MAC
                  address

            For CTC-type partition links, the following properties can be used
            in this method. For a full description, see the description of the
            "Modify Partition Link" operation in the :term:`HMC API` book.
            The provided properties will be added to the request payload of the
            operation, along with the provided partition. The following
            properties can be specified:

              * ``modified-paths`` - array of modified-ctc-path-info objects -
                optional:

                * ``adapter-port-uri`` - URI - required - used to match path
                * ``connecting-adapter-port-uri`` - URI - required - used to
                  match path
                * ``devices`` - array of ctc-endpoint objects - optional:

                  * ``endpoint-pair`` - array of two
                    ctc-partition-devices-endpoint objects - required:

                    * ``partition-uri`` - URI - required - used to match
                      partition
                    * ``device-numbers`` - array of strings - required - device
                      numbers
        ...
        pl_type = self.get_property('type')
        if pl_type == 'ctc':
            body['added-partition-uris'] = [partition.uri]
            body.update(properties)
        else:
            bc = {
                'partition-uri': partition.uri,
            }
            bc.update(properties)
    ...
    def detach_from_partition(
            self, partition, wait_for_completion=True,
            operation_timeout=None):
        """
        ...
        * Object-access permission to this partition link.
        * Object-access permission to the partition to be detached.
        * Task permission to the "Partition Link Details" task.

        Parameters:

          partition (:class:`~zhmcclient.Partition`): The partition from which
            this partition link is to be detached.
        ...
        """
        body = {
            'removed-partition-uris': [partition.uri],
        }
