...
def validate_cert_hostname(cert, hostname):
    ...
    This function uses Python's ssl.match_hostname() which was removed in
    Python 3.7. That function skips the validation when the hostname is
    specified as an IP address, and performs validation only when it is
    specified as a short or long DNS name.
